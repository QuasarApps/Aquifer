...
- Keep tests deterministic: `runTest`, injected `scope` and `WallClock`, Turbine for stream assertions. Fire-and-forget effects (e.g. a stale-while-revalidate refresh) land on the store's injected scope, so drive the scheduler before asserting on them — use the `settle()` helper in `TestHelpers.kt`, a `TestScope` extension over `runCurrent()`. It does not advance virtual time; work gated on a delay needs `advanceUntilIdle()`/`advanceTimeBy(...)`.
