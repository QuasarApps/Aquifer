...
/**
 * Emits once each time [lifecycle] *returns* to the started state — by default the process
 * lifecycle, i.e. the app coming back to the foreground.
 *
 * The launch itself (or a lifecycle that is already started when collection begins) does not
 * emit: the flow fires only for background → foreground transitions observed while
 * collecting. The observer is added while the flow is collected and removed when collection
 * stops; lifecycle access is marshalled to the main thread as the lifecycle API requires.
 */
public fun appForegroundedFlow(
    ...
    onMainThread { lifecycle.addObserver(observer) }
