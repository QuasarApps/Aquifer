 ...
 * header — or its receipt time ([Response.receivedResponseAtMillis]) when `Date` is absent or
 * unusable — to that instant, mirroring how [parseServerFreshness] anchors `Expires`. Every result
 * is floored at [Duration.ZERO] — a past date, or a negative delta, means "retry now" — and any
 * parse failure is absorbed to `null` rather than failing the fetch, since the header is advisory.
 * Delta-seconds is tried first, so a bare number is never misread as a (failing) date.
 *
