...
Central**: the three items that could block publication are cleared, leaving the owner-side signing
secrets and the version bump off `-SNAPSHOT`, plus one step best landed before the tag — cutting a
GitHub Release from it, which the workflow does not yet do. See the roadmap for the rest and for
