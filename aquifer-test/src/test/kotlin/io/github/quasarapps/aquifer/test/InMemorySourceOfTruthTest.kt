...
class InMemorySourceOfTruthTest {
    ...
    fun `latency round-trips, and INFINITE is preserved exactly`() = runTest {
        ...
        // INFINITE is stored verbatim, so it reads back as INFINITE — never confused with a large
        // finite value (the value is kept whole, not reduced to a Long a finite duration could share).
    ...
    fun `a large finite latency completes and never reads back as INFINITE`() = runTest {
        val store = store()
        // A large finite latency must not be mistaken for INFINITE: it reads back finite and, unlike
        // INFINITE's awaitCancellation, actually completes once virtual time reaches it.
